...
module svm_inference_tb;
    ...
    reg signed [WIDTH-1:0] rho_0, rho_1;
    ...
    initial begin
        ...
        // Ground-truth labels
        // ----------------------------------------------------------------
        for (s = 0;  s < 7;  s = s + 1) true_labels[s] = 2'd0;
        for (s = 7;  s < 20; s = s + 1) true_labels[s] = 2'd1;
        for (s = 20; s < 30; s = s + 1) true_labels[s] = 2'd2; // fix: < 30
        ...
        for (s = 0; s < N_SAMPLES; s = s + 1) begin
            ...
            for (v = 0; v < N_SV; v = v + 1) begin
                ...
                alpha_0 = alpha0_mem[v];
                alpha_1 = alpha1_mem[v];
