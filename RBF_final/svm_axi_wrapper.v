...
module svm_axi_wrapper #(
    ...
    always @(posedge axi_clk) begin
        if (rst) begin
            ...
            if (axis_handshake && s_axis_tlast) begin
                tlast_captured <= 1'b1;
            end
            ...
            case (state)
                ...
                // ─────────────────────────────────────────────────
                // TLAST Capture: Global across all loading states
                // ─────────────────────────────────────────────────
                // S_IDLE, S_LOAD_HDR, S_LOAD_SV all have s_axis_tready=1.
                // If handshake occurs and tlast is asserted, capture it.
                ...
                S_WAIT_KERNEL: begin
                    if (k_valid) begin
                        if (sv_idx == N_SV - 1) begin
                            // Last SV: decision engines set done on this
                            // same edge. svm_ready (combinational) will be
                            // valid on the NEXT cycle. Go wait for it.
                            state <= S_DONE_WAIT;
                        end else begin
                ...
                // ─────────────────────────────────────────────────
                // DONE_WAIT: All SVs done — decision_out registers
                //            are now stable. Output the result.
                //            (We arrive here 1 cycle after last k_valid;
                //             done signals are guaranteed high this cycle)
                // ─────────────────────────────────────────────────
                S_DONE_WAIT: begin
                    // Don't wait for svm_ready — go directly to OUTPUT.
                    // decision_out was registered at the previous edge
                    // (when k_valid fired), so predicted_class is stable.
                    m_axis_tvalid <= 1'b1;
